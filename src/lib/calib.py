...
def calibrate_fisheye_camera(
    obj_pts: Array[np.float32, ..., 3], img_pts: Array[np.float32, ..., ..., 2], cam_res: Tuple[int, int]
) -> Union[Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, Array[np.float32, ..., ..., 2]], None]:
...
def calibrate_pair_extrinsics_fisheye(obj_pts, img_pts_1, img_pts_2, k1, d1, k2, d2, cam_res):
    ...
    rms, *_, r, t = cv.fisheye.stereoCalibrate(obj_pts,
                                               img_pts_1,
                                               img_pts_2,
                                               k1,
                                               d1,
...
def calibrate_pairwise_extrinsics(calib_func,
                                  img_pts_arr,
                                  fnames_arr,
                                  k_arr,
                                  d_arr,
...
def adjust_extrinsics_manual_points(calib_func, img_pts_arr, cam_idxs_to_correct, k_arr, d_arr, r_arr, t_arr):
    ...
    def residual_arr(R_t):
        ...
        for a, b in cam_pairs:
            ...
            cam_a_params += [r_arr[a] @ R.T, t_arr[a] -
                             r_arr[a] @ t] if a in cam_idxs_to_correct else [r_arr[a], t_arr[a]]
            cam_b_params = [k_arr[b], d_arr[b]]
            cam_b_params += [r_arr[b] @ R.T, t_arr[b] -
                             r_arr[b] @ t] if b in cam_idxs_to_correct else [r_arr[b], t_arr[b]]

            skew_3d_pts = triangulate_func(np.array(img_pts_arr[:, a]), np.array(img_pts_arr[:, b]), *cam_a_params,
                                           *cam_b_params)
        ...
        for i in range(n_cams):
            ...
            cam_i_params += [r_arr[i] @ R.T, t_arr[i] -
                             r_arr[i] @ t] if i in cam_idxs_to_correct else [r_arr[i], t_arr[i]]
    ...
    n_cams = len(k_arr)
    assert n_cams == img_pts_arr.shape[
        1], 'Number of cams in intrinsic file differs from number of cams in manual points file'
...
def _calibrate_pairwise_extrinsics(calib_func,
                                   camera_fpaths,
                                   points_fpaths,
                                   out_fpath,
    ...
    r_arr, t_arr, incomplete_cams = calibrate_pairwise_extrinsics(calib_func,
                                                                  img_pts_arr,
                                                                  fnames_arr,
                                                                  k_arr,
                                                                  d_arr,
    ...
    if incomplete_cams:
        ...
        r_arr, t_arr = adjust_extrinsics_manual_points(calib_func, img_pts_arr, cam_idxs_to_correct, k_arr, d_arr,
                                                       r_arr, t_arr)
