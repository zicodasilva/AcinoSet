...
def initialize_marker_3d(pts_2d_df, marker, k_arr, d_arr, r_arr, t_arr, dlc_thresh_step=0.01, plot=False, **kwargs):
    ...
    while num_frames == tot_frames:
        pts_3d_df = get_pairwise_3d_points_from_df(pts_2d_df[pts_2d_df['likelihood'] > dlc_thresh],
                                                   k_arr,
                                                   d_arr,
                                                   r_arr,
                                                   t_arr,
    ...
    pts_3d_df = get_pairwise_3d_points_from_df(pts_2d_df[pts_2d_df['likelihood'] > dlc_thresh], k_arr, d_arr, r_arr,
                                               t_arr, triangulate_points_fisheye)
    ...
    if plot:
        ...
        ordinal = {
            ...
            2: 'nd',
            3: 'rd'
        }.get(fit_order % 10 * (fit_order not in [11, 12, 13]),
              'th')  # works up to 110 - https://stackoverflow.com/a/45416102
...
def calibrate_standard_extrinsics_pairwise(camera_fpaths,
                                           points_fpaths,
                                           out_fpath,
...
def sba_board_points_standard(scene_fpath,
    ...
    return _sba_board_points(scene_fpath, points_fpaths, manual_points_fpath, out_fpath, triangulate_func, project_func,
                             camera_indices, manual_points_only)


def sba_board_points_fisheye(scene_fpath,
                             points_fpaths,
                             out_fpath,
                             manual_points_fpath=None,
                             manual_points_only=False,
...
def save_ekf(states, out_dir, scene_fpath, start_frame, dlc_thresh, save_videos=True):
    ...
    save_optimised_cheetah(positions,
                           out_fpath,
                           extra_data=dict(smoothed_positions=smoothed_positions, **states, start_frame=start_frame))
...
def get_vid_info(path_dir, vid_extension='mp4'):
    ...
    if not os.path.isfile(path_dir):
        files = sorted(glob(os.path.join(path_dir,
                                         f'*.{vid_extension}')))  # assume path is a dir that holds video file(s)
        if files:
            path_dir = files[0]
        else:
            raise FileNotFoundError(ENOENT, os.strerror(ENOENT),
                                    orig_path)  # assume videos didn't open due to incorrect path
...
def create_labeled_videos(video_fpaths,
                          videotype='mp4',
                          codec='mp4v',
    ...
    func = partial(proc_video, out_dir, bodyparts, codec, bodyparts2connect, outputframerate, draw_skeleton, pcutoff,
                   dotsize, colormap, skeleton_color)
